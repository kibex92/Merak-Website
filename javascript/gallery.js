...
export class Gallery {
    ...
    createGalleryImagePopup(img) {
        ...
        newImg.onload = () => {
            const newNextBtn = this.createButton('img-btn-next', 'fas fa-chevron-right', () => this.nextImg(1));
            container.appendChild(newNextBtn);

            const newPrevBtn = this.createButton('img-btn-prev', 'fas fa-chevron-left', () => this.nextImg(0));
    ...
    nextImg(next) {
        ...
        let calcNewImg = next === 1 ? this.latestOpenedImg + 1 : this.latestOpenedImg - 1;
        if (calcNewImg > this.galleryImages.length) calcNewImg = 1;
        if (calcNewImg < 1) calcNewImg = this.galleryImages.length;

        const imageFilename = `./images/gallery/img-${calcNewImg}-${this.findClosestFilename(window.innerWidth)}.jpg`;
    ...
    findClosestFilename(deviceWidth) {
        ...
        return availableWidths.reduce((prev, curr) => Math.abs(curr - deviceWidth) < Math.abs(prev - deviceWidth) ? curr : prev);
    }
